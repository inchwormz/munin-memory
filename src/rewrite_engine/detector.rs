...
#[derive(Debug, Clone)]
pub struct CorrectionOccurrence {
    pub wrong_index: usize,
...
pub fn find_corrections(commands: &[CommandExecution]) -> Vec<CorrectionPair> {
    find_correction_occurrences(commands)
        .into_iter()
        .map(|occurrence| occurrence.pair)
        .collect()
}

pub fn find_correction_occurrences(commands: &[CommandExecution]) -> Vec<CorrectionOccurrence> {
    ...
    for i in 0..commands.len() {
        ...
        for (offset, candidate) in commands
            .iter()
            .skip(i + 1)
            ...
            .enumerate()
        {
            ...
            // Found a correction!
            corrections.push(CorrectionOccurrence {
                wrong_index: i,
                right_index: i + 1 + offset,
                pair: CorrectionPair {
